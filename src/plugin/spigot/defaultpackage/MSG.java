...
public enum MSG {
	PLAYER_JOIN (new String[] {"[p] ha acceduto", "[p] è entrato", "Ecco [p] u trmon", "Un caldo benvenuto a [p] u trmon", "Ha trasut [p]", "Awe, cretino [p]!", "Awe u trmongin [p]!", "Salutate tutti la principessa [p] du pisellu!", "Bentornato coglioncino [p]"}),
	PLAYER_LEFT (new String[] {"[p] E' scappato via", "[p] è uscito", "[p] è andato", "[p] aveva qualcosa di meglio da fare", "[p] si sta andando a fare un trimone", "[p] è uscito dal gioco. Pace all'anima sua"}),
	SLEEP (new String[] {"[p] sta dormendo", "[p] s'à  mìs a dorm", "[p] è andato a dormire", "[p] è andato a letto", "[p] dorme", "[p] sta già  russando", "[p] sta già  contando le pecorelle", "[p] ste già jind u litt", "[p] sta già sotto le coperte"}),
	CANT_SLEEP (new String[] {"[p] non puà² dormire per colpa dei mostri"}),
	...
	GOOD_MORNING (new String[] {"Buongiorno trimone", "Buongiorno cazzone", "Buongiornissimo, caffè?", "Buongiorno, ben svegliato", "Buongiorno un cazzo", "Buongiorno principessa!", "Il buongiorno si vede dal mattino", "Buongiorno col cazzo duro"}),
	...
	public String getMessage(Object... params) {
		return String.format(phrases[new Random().nextInt(phrases.length)], params);
	}
}
